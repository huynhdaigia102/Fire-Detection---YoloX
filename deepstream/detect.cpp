...
#define INFER_PGIE_CONFIG_FILE  "/thaint/yolov5/deepstream/configs/infer_x86.txt"
#else
#define INFER_PGIE_CONFIG_FILE "/research/object_detection/common/yolox/deepstream/configs/infer_x86.txt"
#endif

#define TRACKER_CONFIG_FILE "/research/object_tracking/git/ByteTrack/deepstream/configs/tracker.txt"
...
#define PGIE_NET_WIDTH 480
#define PGIE_NET_HEIGHT 288
...
extern "C"
  bool NvDsInferParseYoloX (std::vector < NvDsInferLayerInfo >
...
static GstPadProbeReturn pgie_pad_buffer_probe (GstPad * pad, GstPadProbeInfo * info, gpointer u_data) {
  ...
  for (NvDsMetaList * l_frame = batch_meta->frame_meta_list; l_frame != NULL; l_frame = l_frame->next) {
    ...
    // 1, 1, 1, 1};
    // display_meta->text_params[0].font_params.font_size = 15 * 1.5;
...
#endif
      NvDsInferParseYoloX(outputLayersInfo, networkInfo,
...
int main(int argc, char *argv[]) {
  ...
  /* We need to have a tracker to track the identified objects */
	nvtracker = gst_element_factory_make("nvbytetrack", "tracker");
	nvtracker_queue = gst_element_factory_make("queue", "nvtracker_queue");
  ...
  g_object_set (G_OBJECT (nvtracker), "track-thresh", 0.5, NULL);
  g_object_set (G_OBJECT (nvtracker), "high-thresh", 0.6, NULL);

  // /* Set necessary properties of the tracker element. */
	// if (!set_tracker_properties(nvtracker, TRACKER_CONFIG_FILE))
	// {
	// 	g_printerr("Block DS Main: Failed to set tracker properties. Exiting.");
	// 	return -1;
	// }
  ...
  gst_bin_add_many (GST_BIN (pipeline), nvvidconv, caps_filter, pgie, pgie_queue, nvtracker, nvtracker_queue, tiler, nvosd, NULL);

  if (!gst_element_link_many (streammux, nvvidconv, caps_filter, pgie, pgie_queue, nvtracker, nvtracker_queue, tiler, nvosd, NULL)) {
