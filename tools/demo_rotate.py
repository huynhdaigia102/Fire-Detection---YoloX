...
from yolox.utils import fuse_model, get_model_info, postprocess, vis, vis_rotate

IMAGE_EXT = [".jpg", ".jpeg", ".webp", ".bmp", ".png"]


def npxyxya2vertex(box):
    """
    use radian
    X=x*cos(a)-y*sin(a)
    Y=x*sin(a)+y*cos(a)
...
class Predictor(object):
    ...
    def visual(self, output, img_info, cls_conf=0.35):
        ...
        vis_res = vis_rotate(img, bboxes, scores, cls, cls_conf, self.cls_names)
    ...
    while True:
        ...
        if ret_val:
            ...
            if args.save_result:
                ...
                cv2.imshow("yolox", imutils.resize(result_frame, width=1100))
            ch = cv2.waitKey(1)
            if ch == 27 or ch == ord("q") or ch == ord("Q"):
                break
        else:
