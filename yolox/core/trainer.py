...
class Trainer:
    ...
    def train_in_iter(self):
        self.pbar = tqdm(range(self.max_iter), total=self.max_iter)
        logger.info(('\n' + '%10s' * 11) % ('Epoch', 'iter', 'gpu_mem', 'total', 'iou', 'l1', 'conf', 'cls', 'lr', 'img_size', 'eta'))
        for self.iter in self.pbar:
            self.before_iter()
            self.train_one_iter()
            self.after_iter()
    ...
    def after_iter(self):
        ...
        if ((self.iter + 1) % self.exp.print_interval == 0) or self.iter==0:
            ...
            # logger.info(
            #     "{epoch: {}/{}, iter: {}/{}}, mem: {:.0f}Mb, {}, {}, lr: {:.3e}".format(
            #         progress_str,
            #         gpu_mem_usage(),
            #         time_str,
            #         loss_str,
            ...
            loss_str = [v.latest for v in loss_meter.values()]
            s = ('%10s' * 2 + '%10.4g' * 8 + '%10s') % (
                    f'{self.epoch + 1}/{self.max_epoch}', f'{self.iter + 1}/{self.max_iter}', gpu_mem_usage(), *loss_str, self.meter["lr"].latest, self.input_size[0], eta_str)
